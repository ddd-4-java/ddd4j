 ...
 *
 * <p>Javalin 6 基线：路由注册通过 {@link #applyTo(io.javalin.Javalin)} 实例方法完成。
 */
@Slf4j
public final class Ddd4jJavalinWeb {
    ...
    /**
     * 将 ddd4j 请求处理链安装到 Javalin 实例（javalin 6.x：before/after/exception 注册在实例上）。
     *
     * @param app Javalin 应用实例
     */
    public void applyTo(io.javalin.Javalin app) {
        io.javalin.Javalin javalin = Objects.requireNonNull(app, "app must not be null");
        javalin.before(this::openContext);
        javalin.after(this::completeContext);
        javalin.exception(Exception.class, this::handleException);
        javalin.get(ReadinessEndpoint.PATH, this::readiness);
    }
