 ...
 * Rabbit Message、NATS Message、Rocket MessageExt 等）转成纯 Java {@link MQMessage}。
 *
 * <p>配合 {@link io.ddd4j.mq.ack.MessageAcknowledgment} 的 Factory 模式，
 ...
 */
@FunctionalInterface
public interface MQMessageConverter<N> {

    /**
     * 把 Broker 原生消息转成纯 Java MQMessage。
     *
     * @param nativeMessage Broker 原生消息（如 Kafka ConsumerRecord / Rabbit Message / NATS Message）
     * @return 纯 Java 消息信封
     */
    MQMessage<?> convert(N nativeMessage);
}
