...
public interface MQConsumeInterceptor {
     ...
     */
    default int preCheck(MQConsumerContext context, Message<?> message) {
        return 0;
    }

    /**
     * 消费前检查（兼容旧 {@link MQMessage} 参数）。
     *
     * @param context 消费上下文
    ...
    default int preCheck(MQConsumerContext context, MQMessage<?> message) {
        return preCheck(context, message.toMessage());
    }
     ...
     */
    default void afterConsume(MQConsumerContext context, Message<?> message, AckDisposition disposition) {
        // 默认无操作
    }

    /**
     * 消费完成后回调（兼容旧 {@link MQMessage} 参数）。
     *
     * @param context     消费上下文
     * @param message     消息信封（{@link MQMessage}，兼容旧 API）
     * @param disposition 最终业务处置结果，preCheck 短路时可能为 null
    ...
    default void afterConsume(MQConsumerContext context, MQMessage<?> message, AckDisposition disposition) {
        afterConsume(context, message.toMessage(), disposition);
    }
}
