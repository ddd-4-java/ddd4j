package io.ddd4j.sample.spring.goods.infrastructure;
 ...
 *   <li>实现 {@link Repository}：让 GoodsQuery 的充血查询方法可用</li>
 * </ul>
 *
 * <h3>与 {@code InMemoryOrderRepository} 的对比</h3>
 * <ul>
 *   <li><b>Order 仓储</b>：包含 OrderRow/OrderLineRow 内部 PO 映射层</li>
 *   <li><b>Goods 仓储</b>：直接以 Goods 作为持久化结构，零映射开销</li>
 * </ul>
 *
...
public class InMemoryGoodsRepository implements GoodsRepository, Repository<Goods, Long> {
     ...
     *
     * <p>演示 ddd4j Query 的充血过滤：业务侧只需定义查询字段，
     * 仓储实现根据字段值进行匹配。
     *
     * @param query 查询条件
     * @return 过滤后的商品列表
    ...
    /**
     * 解析查询中的 {@code orderBys} 字段构造比较器。
     *
     * <p>支持格式：{@code "id_ASC,createTime_DESC"} 等，多字段以逗号分隔，
     * 字段名 + 下划线 + 排序方向（ASC/DESC）。
     *
     * @param query 查询条件
     * @return 比较器
     */
    private Comparator<Goods> orderBy(GoodsQuery query) {
        List<LambdaCondition> orderBys = query.getOrderByConditions();
        if (Objects.isNull(orderBys) || orderBys.isEmpty()) {
            // 默认按 id 升序
            return Comparator.comparing(Goods::id);
        }
        Comparator<Goods> comparator = null;
        for (LambdaCondition orderBy : orderBys) {
