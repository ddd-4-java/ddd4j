...
/**
 * RBAC 集成测试（Quarkus + Shiro + JAX-RS）。完整 RBAC 流：
 * <ul>
 *   <li>登录 / 登出 / 当前用户 / 状态</li>
 *   <li>运行时权限 / 角色校验</li>
 *   <li>Shiro 手工鉴权（requireLogin / requireRole / requirePermission / requireRoleAndPermission）</li>
...
class RbacResourceTest {
    ...
    void shouldDenyAccessToAdminEndpointWithoutAdminRole() {
        String token = loginAs("user", "123456");
        Map<String, Object> newUser = createUserBody("u-shiro-test", "shirouser", "s", "user");
        given().header("Authorization", "Bearer " + token)
    ...
    void shouldCreateUserAsAdmin() {
        String token = loginAs("admin", "123456");
        String userId = "u-shiro-create-" + System.nanoTime();
        Map<String, Object> newUser = createUserBody(userId, "shiroNew", "Shiro 新", "user");
        given().header("Authorization", "Bearer " + token)
                .contentType("application/json").body(newUser)
                .when().post("/admin/users")
                .then()
                ...
                .body("data.username", equalTo("shiroNew"));
    }
    ...
    void shouldUpdateUserAsAdmin() {
        String token = loginAs("admin", "123456");
        String userId = "u-shiro-update-" + System.nanoTime();
    ...
    void shouldDeleteUserAsAdminViaAdminEndpoint() {
        String token = loginAs("admin", "123456");
        String userId = "u-shiro-del-" + System.nanoTime();
        Map<String, Object> newUser = createUserBody(userId, "deluser", "Shiro 删除", "user");
        given().header("Authorization", "Bearer " + token)
                .contentType("application/json").body(newUser)
                .when().post("/admin/users")
                .then().statusCode(200);
    ...
    void shouldAssignRoleToUserAsAdmin() {
        String token = loginAs("admin", "123456");
        String userId = "u-shiro-assign-" + System.nanoTime();
        Map<String, Object> newUser = createUserBody(userId, "assignuser", "分配", new HashSet<>());
        given().header("Authorization", "Bearer " + token)
                .contentType("application/json").body(newUser)
                .when().post("/admin/users")
                .then().statusCode(200);
    ...
    void shouldRevokeRoleFromUserAsAdmin() {
        String token = loginAs("admin", "123456");
        String userId = "u-shiro-revoke-" + System.nanoTime();
        Map<String, Object> newUser = createUserBody(userId, "rev", "解除", "admin");
        given().header("Authorization", "Bearer " + token)
                .contentType("application/json").body(newUser)
                .when().post("/admin/users")
                .then().statusCode(200);
    ...
    void shouldCreateRoleAsAdmin() {
        String token = loginAs("admin", "123456");
        String roleCode = "shiro-test-" + System.nanoTime();
        Map<String, Object> role = new HashMap<>();
        role.put("code", roleCode);
        role.put("displayName", "Shiro 测试角色");
        role.put("description", "for shiro test");
    ...
    void shouldUpdateRoleAsAdmin() {
        String token = loginAs("admin", "123456");
        String roleCode = "shiro-upd-" + System.nanoTime();
        Map<String, Object> role = new HashMap<>();
        role.put("code", roleCode);
    ...
    void shouldDeleteRoleAsAdmin() {
        String token = loginAs("admin", "123456");
        String roleCode = "shiro-del-" + System.nanoTime();
        Map<String, Object> role = new HashMap<>();
        role.put("code", roleCode);
    ...
    void shouldCreatePermissionAsAdmin() {
        String token = loginAs("admin", "123456");
        String permCode = "shiro:perm-" + System.nanoTime();
        Map<String, Object> perm = new HashMap<>();
        perm.put("code", permCode);
        perm.put("displayName", "Shiro 测试权限");
        perm.put("description", "created from shiro test");
        given().header("Authorization", "Bearer " + token)
    ...
    void shouldDeletePermissionAsAdmin() {
        String token = loginAs("admin", "123456");
        String permCode = "shiro:del-" + System.nanoTime();
        Map<String, Object> perm = new HashMap<>();
        perm.put("code", permCode);
