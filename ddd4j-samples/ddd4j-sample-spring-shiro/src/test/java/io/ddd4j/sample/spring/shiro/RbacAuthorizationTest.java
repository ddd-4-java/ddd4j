...
/**
 * AuthorizationController 授权管理 CRUD 测试（Shiro）。
 *
 * @author Test
 */
@SpringBootTest
@AutoConfigureMockMvc
@DisplayName("授权管理 CRUD - Shiro")
class RbacAuthorizationTest {
    ...
    void updateUser_modifiesFields() throws Exception {
        ...
        mockMvc.perform(put("/admin/users/" + userId)
                        ...
                        .content(objectMapper.writeValueAsString(
                                new io.ddd4j.sample.spring.shiro.controller.AuthorizationController.UpdateUserRequest(
                                        "Renamed", null, null))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    @Test
    @DisplayName("POST /admin/users/{id}/roles - 分配角色")
    void assignRolesToUser_replacesAll() throws Exception {
        String userId = "TU-R-" + System.nanoTime();
        User u = new User(userId, "toassign", "pwd", "Assigner", User.Status.ENABLED);
        u.assignRoles(new HashSet<>(List.of("R001")));
        userRepository.save(u);

        mockMvc.perform(post("/admin/users/" + userId + "/roles")
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(
                                new io.ddd4j.sample.spring.shiro.controller.AuthorizationController.AssignRolesRequest(
                                        List.of("R002", "R003")))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    @Test
    @DisplayName("POST /admin/users/{id}/roles - 不存在 → 业务异常")
    ...
    @Test
    @DisplayName("GET /admin/users/{id}/permissions - 角色+权限")
    ...
    void updateRole_modifiesFields() throws Exception {
        ...
        roleRepository.save(new Role(roleId, "test-role-upd", "Old", "Desc", Role.Status.ENABLED));
        mockMvc.perform(put("/admin/roles/" + roleId)
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(
                                new io.ddd4j.sample.spring.shiro.controller.AuthorizationController.UpdateRoleRequest(
    ...
    @Test
    @DisplayName("POST /admin/roles/{id}/permissions - 分配权限")
    void assignPermissionsToRole_replacesAll() throws Exception {
        ...
        mockMvc.perform(post("/admin/roles/" + roleId + "/permissions")
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(
                                new io.ddd4j.sample.spring.shiro.controller.AuthorizationController.AssignPermissionsRequest(
                                        List.of("P001", "P003")))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    void updatePermission_modifiesFields() throws Exception {
        ...
        mockMvc.perform(put("/admin/permissions/" + permId)
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(
                                new io.ddd4j.sample.spring.shiro.controller.AuthorizationController.UpdatePermissionRequest(
                                        "Renamed", "test", null))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
