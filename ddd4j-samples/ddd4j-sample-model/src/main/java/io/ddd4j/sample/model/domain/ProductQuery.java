 ...
 * <ul>
 *   <li>{@link #page()} - 分页查询，返回 {@code Page<Product>}</li>
 *   <li>{@link #list()} - 列表查询，返回 {@code List<Product>}</li>
 ...
 * <pre>{@code
 * // 1. 链式构建
 * Page<Product> page = new ProductQuery()
 *     .setKeyword("iPhone")
 ...
 *
 * // 2. 简单计数
 * long count = new ProductQuery().setStatus(ProductStatus.ON_SALE).count();
 *
 * // 3. 单条查询
 * Product product = new ProductQuery().setCode("SKU-001").one();
 * }</pre>
 *
...
public class ProductQuery extends Query<Product> {
    ...
    /**
     * 绑定到 {@link Product} 聚合根的仓储实例。
     * <p>
     * {@link Query#page()} / {@link #list()} / {@link #one()} 等充血查询方法
     * 会通过本方法获取仓储，再委托给 {@link ProductRepository}。
     *
     * @return {@link Product} 聚合根的仓储实例
     */
    @Override
    protected Repository repository() {
        return RepositoryRegistry.repository(Product.class);
    }
}
