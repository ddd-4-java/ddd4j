 ...
 * 与 {@code ddd4j-sample-rich-model} 中的 {@code Order} 聚合形成对比。
 *
 * <p>实体仍继承 {@link AggregateRoot}，因为 ddd4j 的 {@code Repository<M, ID>}
 * 约束仓储只针对聚合根——这是 ddd4j 唯一的领域模型基类。
 * 但本示例的 Product 不会调用任何充血方法（如 {@code save()} / {@code delete()}），
 * 持久化由应用服务统一编排。
 *
...
public class Product extends AggregateRoot<Long> implements Serializable {
