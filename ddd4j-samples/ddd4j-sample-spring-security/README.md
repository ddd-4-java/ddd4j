# ddd4j-sample-spring-security

## 鉴权示例（Spring Boot + Spring Security）三轨合一

ddd4j SubjectKit 统一鉴权入口在 Spring Boot 框架下的 Spring Security 适配示例。
...
- **切换底层鉴权框架只需改 pom.xml**：从 shiro 切换到 spring-security，只需替换 2 个依赖，业务代码不动
...
├── pom.xml                              # 依赖配置
└── src/
    ├── main/java/io/ddd4j/sample/spring/security/
    │   ├── SpringSecurityApplication.java      # 启动类
    ...
    ├── main/resources/
    │   └── application.yml
    └── test/java/io/ddd4j/sample/spring/security/
        └── AuthControllerTest.java
```

### 🔗 相关示例

- 同一框架的其他认证方案：`ddd4j-sample-spring-satoken`、`ddd4j-sample-spring-shiro`
- 其他框架的同认证方案：暂无 Javalin/Quarkus 的 Security 适配示例
