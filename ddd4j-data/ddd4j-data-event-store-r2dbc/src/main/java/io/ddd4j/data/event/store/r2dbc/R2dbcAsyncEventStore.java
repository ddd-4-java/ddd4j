 ...
 *
 * <h3>payload</h3>
 * <p>经 {@link EventPayloadSerializer}（Jackson 3，无多态标记的安全序列化契约）
 * 序列化/还原；事件类缺失（被删除/重命名）时以明确错误信号失败——异步轨道的
 * {@link AsyncStoredEvent#payload()} 要求类型化 {@link DomainEvent}，不做 Map 回退。
 *
 * <h3>时间戳</h3>
 * <p>与同步轨道一致：写入 {@code LocalDateTime.now()}，读取以系统时区还原
 * {@link ZonedDateTime}（instant 语义不变）。
 *
...
public class R2dbcAsyncEventStore implements AsyncEventStore {
     ...
     * @param payloadSerializer payload 序列化器（Jackson 3 mapper 的副本隔离语义见其构造器）
     */
