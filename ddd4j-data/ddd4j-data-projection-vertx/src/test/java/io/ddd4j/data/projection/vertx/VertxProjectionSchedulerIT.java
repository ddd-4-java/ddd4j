...
/**
 * {@link VertxProjectionScheduler} + {@link VertxProjectionViewManager}
 * 真实 Vert.x 实例生命周期集成测试。
 *
 * <p>真实 {@code Vertx.vertx()} 实例（IT 全程生命周期自管）＋手动装配链
 * （Vert.x 无容器——集成方直接构造或经工厂创建）：构造
 * {@link VertxProjectionScheduler} + 测试替身（ProjectionService /
 * EventChunkReader / ProjectionRunner / ProjectionView）+ 经
 * {@link VertxProjectionViewManager#create} 工厂装配视图管理器，
 * 完整验证 ViewScheduler 的 schedule/cancel/isActive 契约与 ViewManager 的
 * start/stop/isRunning/triggerOnce 生命周期。三用例与 Quarkus / Helidon / Javalin
 * IT 对齐：
 * <ol>
 ...
 * @since 2.0.x
 */
@DisplayName("VertxProjectionScheduler + ViewManager 真实 Vert.x 实例生命周期 IT")
class VertxProjectionSchedulerIT {
