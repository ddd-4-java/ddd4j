...
/**
 * 基于 Quarkus Hibernate ORM Panache 的 {@link EventStore} 实现（ADR-0005，见
...
public class PanacheEventStore implements EventStore {
     ...
     * 也会让重复版本号插入失败（第二道，数据层兜底，ADR-0005）。全程运行在同一事务内
     ...
     * 原值与 {@code 类型:值} 形式（与 -jpa 模块 {@code JpaEventStore} 内同名适配器对齐）。
     */
