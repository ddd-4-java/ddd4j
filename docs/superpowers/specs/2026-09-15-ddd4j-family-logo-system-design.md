...
1. 三个纯色等距块面组成一个开放六边形，分别表示 Domain、Application、Infrastructure。
2. 三个块面之间只使用一道白色 Y 形负空间表达 COLA 分层与依赖边界，不再叠加内框。
3. 统一 30° 等距透视、统一圆角、统一间距和统一平面光感；禁止外环、外部节点和装饰底座。
4. 右下主题块面只承载一个组件专属主符号；不得再增加第二组漂浮符号。
...
- 不使用照片级 3D、渐变、复杂阴影、外环、外部节点、嵌套边框、独立徽章底座、随机字体或每个项目不同的图标容器。
