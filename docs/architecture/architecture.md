...
> 本文档综合 `codegraph` 索引（689 文件 / 11,625 符号 / 21,930 边）与全部架构剖析文档，形成 ddd4j 项目的**最终架构定位**与**模块全景**。
...
**12 种 MQ 实现**：Kafka / RabbitMQ / RocketMQ (ONS) / ActiveMQ / RedisStream / MQTT / MicaMQTT / Pulsar / NATS / Disruptor（进程内高性能）
...
public interface BaseRepository<M extends Model, Q extends Query> {
    ...
    boolean save(M model); boolean update(M model); ...
    Page<M> page(Q query); List<M> list(Q query);
...
public interface Repository<M, Q, P extends Serializable> {
    ...
    M save(M entity); M updateById(M entity);
}
```
