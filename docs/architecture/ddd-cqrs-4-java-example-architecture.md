...
是 ddd4j 实现 `ddd4j-quarkus` 和 `ddd4j-runtime-spring` 适配层的**最佳参考实现**。
...
- `ddd4j-runtime-spring` 与 `ddd4j-quarkus`（及 `ddd4j-javalin`）下用同一套共享契约
...
- **SpringJpaViewManager** → 抽取到 `ddd4j-runtime-spring` 作为默认视图管理器
...
4. **借鉴其双框架对照**为 ddd4j-runtime-spring / ddd4j-quarkus / ddd4j-javalin 提供平行参考实现
