...
./mvnw -B -ntp -DskipTests org.owasp:dependency-check-maven:12.2.2:aggregate -DnvdApiKey="${NVD_API_KEY}" -Dformat=ALL -DoutputDirectory="${CVE_DIR}"
find "${CVE_DIR}" -maxdepth 1 -type f -name 'dependency-check-report.*' -size +0c -print -quit | grep -q .
echo "[PASS] CVE reports generated in ${CVE_DIR}; no CVSS threshold is enforced yet."
