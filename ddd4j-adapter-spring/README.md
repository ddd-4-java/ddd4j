...
| **SPI 默认实现层**    | 三框架各自的 SPI 实现（无 starter）                                         | `ddd4j-adapter-spring` / `ddd4j-adapter-quarkus` / `ddd4j-adapter-guice` |
| **自动装配 / 胶水代码层** | Spring Boot starter / `@AutoConfiguration` / `spring.factories` | `ddd4j-boot-ddd-autoconfigure` 等                 |

> **错误做法**：把 `DddAutoConfiguration`（含 `@Bean` 装配 EventStore/MultiCommandExecutor）放进 `ddd4j-adapter-spring`
> **正确做法**：`ddd4j-adapter-spring` 只提供 `SpringDomainEventPublisher` 这种**带 `@Component` 的 Bean 实现**；
...
└──────────────────────────────┬───────────────────────────────────┘
                               │ 依赖 ddd4j-adapter-spring（纯 SPI 实现）
┌──────────────────────────────▼───────────────────────────────────┐
│                          ddd4j-adapter-spring                             │
│  ┌────────────────────────────────────────────────────────────┐  │
...
| 维度       | 数据                                                                     |
|----------|------------------------------------------------------------------------|
| **路径**   | `ddd4j-adapter-spring/src/main/java/io/ddd4j/spring/`                          |
...
> `@EnableDdd4j` 属于具体框架脚手架的启用入口。`ddd4j-adapter-spring` / `ddd4j-adapter-quarkus` / `ddd4j-adapter-guice` 只提供可复用 SPI 实现，不承载 Spring Boot starter 或自动装配。

---

## 三、SPI 桥接层 — 把纯 Java 契约落地到 Spring

`ddd4j-core` 定义了三组**框架无关的 SPI 接口**，`ddd4j-adapter-spring` 提供 Spring 实现：
...
## 八、与 ddd4j-adapter-quarkus / ddd4j-javalin 的对照
...
`ddd4j-adapter-spring` 在三框架中**工具最丰富、集成度最高**（因为 Spring 生态最成熟），但**业务代码**通过 SPI 调用，**与
